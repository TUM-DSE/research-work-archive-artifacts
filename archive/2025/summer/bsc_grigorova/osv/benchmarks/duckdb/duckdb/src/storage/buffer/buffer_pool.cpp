...
void BufferPool::MemoryUsage::UpdateUsedMemory(MemoryTag tag, int64_t size) {
	...
	update_used_memory_ctx ctx = {
		(uint64_t) tag,
		size,
		(uint64_t)MEMORY_USAGE_CACHE_THRESHOLD,
		(uint64_t)MEMORY_USAGE_CACHE_COUNT,
		(uint64_t)(MEMORY_TAG_COUNT + 1),
		(uint64_t)TOTAL_MEMORY_USAGE_INDEX,
		(uint64_t)(uintptr_t) &memory_usage_caches[0][0],
		(uint64_t)(uintptr_t) &memory_usage[0]

	 };
	if (call_hook("update_used_memory", &ctx, sizeof(ctx), &result) ==0){
	return;
	}
#endif
