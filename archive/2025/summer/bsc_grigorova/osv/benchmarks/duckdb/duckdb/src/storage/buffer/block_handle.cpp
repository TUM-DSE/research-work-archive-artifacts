...
void BlockHandle::ReadTemporaryBufferHelper(unique_ptr<FileBuffer> reusable_buffer){
...
BufferHandle BlockHandle::Load(unique_ptr<FileBuffer> reusable_buffer) {
	...
	block_handle_load_ctx ctx = {(uint64_t)(uintptr_t)this, (uint64_t)(uintptr_t)&buffer, 
									(uint64_t)(uintptr_t)&reusable_buffer };

	if(call_hook("load", &ctx, sizeof(ctx), &result)==0){
