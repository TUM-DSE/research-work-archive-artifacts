...
void ArenaAllocator::PrependChunk(idx_t capacity) {
...
void ArenaAllocator::AllocateNewBlock(idx_t min_size) {
	...
	arena_allocator_allocate_ctx ctx = {(uint64_t)(uintptr_t)this, min_size, ARENA_ALLOCATOR_MAX_CAPACITY };

	if (call_hook("allocate", &ctx, sizeof(ctx), &result)==0){
	allocated_size += (idx_t) result;
	return;
	}
#endif 
