...
public:
friend bool add_to_eviction_queue(uint64_t bm,  uint64_t handle, uint64_t lock);
friend void verify_zero_readers(uint64_t bm,  uint64_t handle, uint64_t lock);
friend void bpf_purge_block(uint64_t bm,  uint64_t handle);
