...
extern int call_hook(const char* hook_name, void *ctx, size_t ctx_size, uint64_t *result);
...
void StandardBufferManager::Unpin(shared_ptr<BlockHandle> &handle) {
	...
	{
	auto lock = handle->GetLock();
	unpin_hook_ctx ctx = {(uint64_t)(uintptr_t)this, (uint64_t)(uintptr_t)&handle, (uint64_t)(uintptr_t)&lock};
	call_result = call_hook("unpin", &ctx, sizeof(ctx), &result);
	}
	if(call_result==0){
	if(result==0){
		PurgeQueue(*handle);
