...
void TaskScheduler::EnqueueHelper( ProducerToken &token, shared_ptr<Task> task) {
    // the *only* place we touch scheduler->queue
...
void TaskScheduler::ScheduleTask(ProducerToken &token, shared_ptr<Task> task) {
	#ifdef HOOKS
	schedule_task_ctx ctx = {(uint64_t)(uintptr_t)this,         // address of the real scheduler
        (uint64_t)(uintptr_t)&token,       // address of the ProducerToken
        (uint64_t)(uintptr_t)&task         // address of this local shared_ptr<Task>
    };        // address of your local shared_ptr<Task>};
	uint64_t hook_ret = 0;

    if(call_hook("schedule_task", &ctx, sizeof(ctx), &hook_ret)==0){
	return;
	}
...
void TaskScheduler::QueueWaitHelper() {
    queue->semaphore.wait();
}

bool TaskScheduler::QueueWaitTimeoutHelper(int64_t timeout_us) {
    return queue->semaphore.wait(timeout_us);
}

bool TaskScheduler::TryDequeueHelper(shared_ptr<Task> &task) {
    return queue->q.try_dequeue(task);
}

void TaskScheduler::FlushHelper(uint64_t threshold, uint64_t requested_thread_count) {
    Allocator::ThreadFlush(allocator_background_threads, threshold,
			                       NumericCast<idx_t>(requested_thread_count));
}
...
#ifdef HOOKS
	uint64_t result;
	execute_forever_ctx ctx{
            (uint64_t)(uintptr_t)this,
            Allocator::SupportsFlush() ? 1ull : 0ull,
            (uint64_t)allocator_flush_threshold.load(),  
            (uint64_t)requested_thread_count.load(),
            (uint64_t)(uintptr_t)&task
    };
    while (*marker) {
		ctx.flush_threshold       = (uint64_t)allocator_flush_threshold.load();
    	ctx.requested_threads= (uint64_t)requested_thread_count.load();
        call_hook("execute_forever", &ctx, sizeof(ctx), &result);
    }
