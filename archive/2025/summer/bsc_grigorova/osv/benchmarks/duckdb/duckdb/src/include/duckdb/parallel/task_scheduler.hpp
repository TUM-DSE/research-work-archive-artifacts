...
public:
	...
	void QueueWaitHelper();
	bool QueueWaitTimeoutHelper(int64_t timeout_us);
	bool TryDequeueHelper(shared_ptr<Task> &task);
	void FlushHelper(uint64_t threshold, uint64_t requested_thread_count);
	void EnqueueHelper( ProducerToken &token, shared_ptr<Task> task);
	
	ConcurrentQueue &GetInternalQueue() {
        return *queue;
    }
