...
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "translated_execs.txt")
...
with open(OUTPUT_FILE, "w") as out:
    for c_file in sorted(glob.glob(os.path.join(TESTS_DIR, "*.c"))):
        ...
        # Step 1: Compile to ARM binary
        compile_cmd = gcc_base + [c_file, polybench_src, "-I", polybench_inc, "-o", arm_binary]
        result = subprocess.run(compile_cmd, capture_output=True, text=True)
        if result.returncode != 0:
            line = f"{benchmark}: COMPILATION FAILED\n{result.stderr}"
            print(line)
            out.write(line + "\n")
            continue

        # Step 2: Translate ARM -> x86
        translate_cmd = [SGTRANSLATOR, arm_binary, "--config", CONFIG]
        result = subprocess.run(translate_cmd, capture_output=True, text=True)
        if result.returncode != 0:
            line = f"{benchmark}: TRANSLATION FAILED\n{result.stderr}"
            print(line)
            out.write(line + "\n")
            continue

        # Step 3: Execute and time
        times = []
        for i in range(RUNS):
            start = time.perf_counter()
            subprocess.run([x86_binary], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        ...
        # Cleanup
        for f in [arm_binary, x86_binary]:
            try:
                os.remove(f)
