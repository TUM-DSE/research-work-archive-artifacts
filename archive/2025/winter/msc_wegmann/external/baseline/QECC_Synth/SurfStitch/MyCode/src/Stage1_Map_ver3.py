...
class Stage1_Solver():
    def __init__(self, code, CG, protocal, maxL, Len, wcnfName, solName, timeout=None, preData=None) -> None:
    ...
    def writeOptimizationConstraints(self):
        ...
        # P0: Match Previous data map
        if self.preData:
            for i in range(self.dataNum):
                self.writeSoftClause(int(w * math.sqrt(self.physNum) * 4), [
                    [('q', i, self.preData[i]), True]
                ])
        ...
        while not result:
            ...
            if SAT_GLOBAL:
                ...
                try:
                    ...
                    p = subprocess.Popen(['external/baseline/QECC_Synth/SurfStitch/MyCode/lib/NuWLS-c/bin/nuwls-c_static', self.wcnfName],  stdout=open(self.solName, 'w'))
                    print("started")
                    ...
                    #p = subprocess.Popen(cmd, stdout=open(self.solName, 'w'), shell=True)
...
def S1_transpile(code, CG, protocal, maxL, Len, chunkNum, wcnfName, solName, S1_resultName):
    dataNum, stabilizers = code
    stabNum = len(stabilizers)
    chunkSize = stabNum // chunkNum
    larger_count = stabNum % chunkNum
    ...
    for _ in range(roundNum):
        ...
        for k in range(chunkNum):
            end = start + chunkSize + (1 if k < larger_count else 0)
            Stab.append(stabilizers[start:end])
            start = end

        for k in range(chunkNum):
            print("\n Chunk %d:\n" % k)
            solver = Stage1_Solver((dataNum, Stab[k]), CG, protocal, maxL, Len, wcnfName, solName, preData=preData)
            solver.solve()
            Data.append(solver.Data)
            # Stab.append(solver.Stab)
            Collison_list.append(solver.Collison_list)
            preData = solver.Data
